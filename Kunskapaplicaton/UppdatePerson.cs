...
{
    internal class UppdatePerson : IUppdatePrson
    {
        private readonly PersonDbContext _personDbContext;

        public UppdatePerson(PersonDbContext db)
        {
            this._personDbContext = db;
        }
        public void Uppdate(usersinfor usersinfor)
        {
            var query = "Uppdate usersinfor(id, first_name, last_name, title) values(@id, @first_name, @last_name, @title)";
